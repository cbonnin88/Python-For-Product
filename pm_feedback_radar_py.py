...
if total_reviews > 0:
    # Step A: Group by score and count rows using Polars
    score_dist = filtered_product_df.group_by("score").len().sort("score")

    # Step B: Convert to Pandas dataframe because Altair requires Pandas
    chart_data = score_dist.to_pandas()

    # Step C: Build a custom Altair chart
    # - mark_bar(): Tells Altair to draw vertical bar columns
    # - encode(): Maps our data columns to visual axes
    # - "score:O": The ':O' means Ordinal (ordered categories 1 through 5)
    # - labelAngle=0: THIS IS THE MAGIC FIX! Forces text to be horizontal (0 degrees)
    # - tooltip: Shows exact numbers when you hover your mouse over the bars
    chart = (
        alt.Chart(chart_data)
        .mark_bar(color="#FF4B4B")  # Uses a clean Streamlit-red accent color
        .encode(
            x=alt.X(
                "score:O",
            ...
            tooltip=[
                alt.Tooltip("score:O", title="Star Rating"),
                alt.Tooltip("len:Q", title="Review Count")
            ]
        )
        .properties(height=350)
    )

    # Step D: Render the custom chart in Streamlit
